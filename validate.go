...
func (ctx *ValidationContext) findSignature(root *etree.Element) (*Signature, error) {
	return ctx.findSignatureIn(root, nil)
}

// findSignatureIn behaves like findSignature but, when only is non-nil,
// considers solely that element as the Signature candidate.
func (ctx *ValidationContext) findSignatureIn(root, only *etree.Element) (*Signature, error) {
	...
	// of the root.
	if sig == nil {
		limit := ctx.MaxTraversalElements
		if only != nil {
			limit = -1
		}
		nsctx := etreeutils.NewDefaultNSContext()
		if limit != 0 {
			nsctx = etreeutils.NewNSContextWithLimit(limit)
		}
...
// enveloped Signature element themselves: only that element is considered,
// and the signature-search traversal budget does not apply, which makes
// validation of very large documents (e.g. ETSI trusted lists) independent
// of any traversal limit.
//
// sig must be a ds:Signature element inside root's tree (and not root
// itself). Like Validate, the passed elements are never mutated: root is
// defensively copied and sig is resolved inside the copy by its position.
// All other behaviour is identical to Validate — the signature must carry a
// reference to the top-level element (or the document must be id-less,
// mirroring the search's fallback), the KeyInfo certificate is checked
// against the CertificateStore, and every SignedInfo reference digest is
// verified.
func (ctx *ValidationContext) ValidateSignature(root, sig *etree.Element) ([]*etree.Element, error) {
	...
	// mapPathToElement only matches proper descendants, so this also rejects
	// sig == root and elements from a different tree.
	...
	signature, err := ctx.findSignatureIn(rootCopy, sigCopy)
	if err != nil {
		return nil, err
	}
